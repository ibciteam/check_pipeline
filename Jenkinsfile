pipeline {
  ...
  stage("Build CVE job"){
      ...
      build job: 'run_docker_image_cve_scan', parameters: [[$class: 'StringParameterValue', name: 'upstream_git_commit', value:env.GITHUB_COMMIT], [$class: 'StringParameterValue', name: 'upstream_github_repo', value:env.GIT_REPO],[$class: 'StringParameterValue', name: 'upstream_build_PR', value:env.GITHUB_PR], [$class: 'StringParameterValue', name: 'upstream_job_name', value:$JOB_NAME], [$class: 'StringParameterValue', name: 'upstream_job_owners', value:"${ownership.job.primaryOwnerEmail}"], [$class: 'StringParameterValue', name: 'upstream_build_user', value:$BUILD_USER_EMAIL], [$class: 'StringParameterValue', name: 'upstream_execution_url', value:$BUILD_URL], [$class: 'StringParameterValue', name: 'tag', value:'20190828-45-357a348'], [$class: 'StringParameterValue', name: 'repo', value:'infobloxcto/siemserver']]
      }
  }  
